...
function fix_mobile_nav(){if(jQuery(window).width()<=768){var e=jQuery(".upper-header").height();$(".header-inner").addClass("fixed"),$(".banner-wrapper").css("margin-top",e)}else $(".header-inner").removeClass("fixed"),$(".banner-wrapper").css("margin-top","")}function fix_grid_height(){if(jQuery(".fix-height").length>0&&(jQuery(".fix-height").height("auto"),jQuery(window).width()>=768)){if(jQuery(".fix-height:not([data-fix-height-group])").length>0){var e=new Date,i=e.getTime();jQuery(".fix-height:not([data-fix-height-group])").each(function(){jQuery(this).attr("data-fix-height-group",i)})}for(var n=jQuery(".fix-height").map(function(){return jQuery(this).attr("data-fix-height-group")}).get(),t=n.filter(only_unique_values),a=t.length-1;a>=0;a--){var o=jQuery('.fix-height[data-fix-height-group="'+t[a]+'"]'),r=o.map(function(){return jQuery(this).height()}).get(),u=Math.max.apply(null,r);o.height(u)}}}function only_unique_values(e,i,n){return n.indexOf(e)===i}function gmap_init(){if($("#gmap").length>0){var e={lat:1.303173,lng:103.824202},i=new google.maps.Map(document.getElementById("gmap"),{zoom:17,center:e}),n=new google.maps.InfoWindow({content:"Gynae MD Women's & Rejuvenation Clinic<br>#04-03A Camden Medical Centre<br>1 Orchard Boulevard Singapore 248649"}),t=new google.maps.Marker({position:e,map:i,title:"Gynae MD Women's & Rejuvenation Clinic"});t.addListener("click",function(){n.open(i,t)}),n.open(i,t)}}$(function(){"use strict";gmap_init(),fix_mobile_nav(),setTimeout(function(){fix_grid_height()},2e3),jQuery(window).resize(function(){fix_grid_height(),fix_mobile_nav()}),$(".sidebar-accordion .menu").navgoco({accordion:!0}),$(".close-popup").click(function(){var e=$(this).data("popup-target");$('.popup[data-popup="'+e+'"]').fadeOut()}),$(window).bind("scroll",function(){fix_mobile_nav()})});